...
import { Bell, BellSlash, Check, CheckAll, Trash, Trash2 } from 'react-bootstrap-icons';
import { useNavigate } from 'react-router-dom';
import useNotificationSocket from '../hooks/useNotificationSocket';
import toast from '../utils/toast';
import '../pages/shared/NotificationPage.css';

const ITEMS_PER_PAGE = 15;

export default function BaseNotificationPage({
  iconMap = {},
  onNotificationClick,
  socketEnabled = false,
  toastEnabled = false
  ...
  // Real-time notification (Socket.io) — only when socketEnabled
  ...
  useNotificationSocket(socketEnabled ? socketCallback : null, socketEnabled);
  ...
  const handleMarkAsRead = async (id) => {
    try {
      ...
      if (toastEnabled) toast.error('ทำเครื่องหมายอ่านแล้วไม่สำเร็จ');
    }
  };
  ...
  const handleMarkAllAsRead = async () => {
    try {
      ...
      if (toastEnabled) toast.error('ทำเครื่องหมายอ่านทั้งหมดไม่สำเร็จ');
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (toastEnabled) toast.success('ลบการแจ้งเตือนแล้ว');
    } catch {
      if (toastEnabled) toast.error('ลบไม่สำเร็จ');
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      setSelectedIds(new Set());
      if (toastEnabled) toast.success(`ลบ ${count} รายการแล้ว`);
    } catch {
      if (toastEnabled) toast.error('ลบไม่สำเร็จ');
    }
  };
  ...
  const handleClickNotification = (notif) => {
    ...
    if (onNotificationClick) {
      onNotificationClick(notif, navigate);
    } else if (notif.link) {
      navigate(notif.link);
    }
  };
            ...
            return (
              ...
              >
                ...
                <div 
                  ...
                  style={{ cursor: (notif.link || onNotificationClick) ? 'pointer' : 'default' }}
                >
                  <div 
